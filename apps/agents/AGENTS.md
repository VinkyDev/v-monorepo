...
[Mastra](https://mastra.ai) agents. Register each agent in `src/mastra/index.ts`. Models use `"provider/model"`; credentials come from Mastra's provider env vars (`OPENAI_API_KEY`, optional `OPENAI_BASE_URL`, …). `DATABASE_URL` is validated by T3 Env in `src/env.ts`. MCP servers live in `src/mastra/mcp.ts` (Wikipedia, no extra keys). Attach skills with `createSkill` from `@mastra/core/skills`.

...
import { Input } from "@v-monorepo/ui/components/input";
...
const lastMessageAtFromCustom = (custom: unknown): Date | undefined => {
  if (typeof custom !== "object" || custom === null) {
    return undefined;
  }
  if (
    !("lastMessageAt" in custom) ||
    typeof custom.lastMessageAt !== "string"
  ) {
    return undefined;
  }
  if (custom.lastMessageAt.length === 0) {
    return undefined;
  }
  const date = new Date(custom.lastMessageAt);
  return Number.isNaN(date.getTime()) ? undefined : date;
};

const itemDate = (
  item: { custom?: unknown; lastMessageAt?: Date } | undefined
): Date | undefined =>
  item?.lastMessageAt ?? lastMessageAtFromCustom(item?.custom);
...
export const useThreadListGroups = (searchQuery = "") => {
  ...
  return useMemo(() => {
    ...
    const dates = threadIds.map((id) => itemDate(itemsById.get(id)));
    const filteredIndices = threadIds
      .map((id, index) => ({ id, index }))
      .filter(
        ({ id }) =>
          !query ||
          (itemsById.get(id)?.title || "新对话").toLowerCase().includes(query)
      )
      .map(({ index }) => index);
    if (!filteredIndices.some((index) => dates[index])) {
      return { filteredIndices, groups: null, threadIds };
    }
    ...
    for (const index of sorted) {
      ...
      if (lastGroup?.label === label) {
        ...
        result.push({ indices: [index], label });
      }
    }
    return { filteredIndices, groups: result, threadIds };
...
const ThreadListItemGroups: FC<{ searchQuery?: string }> = ({
  ...
  const { filteredIndices, groups, threadIds } =
    ...
    >
      {isRenaming ? (
        ...
        >
          ...
          {isRunning && <span className="sr-only">进行中</span>}
        </ThreadListItemPrimitive.Trigger>
      )}
...
const ArchivedThreadListItem: FC = () => {
  return (
    ...
    >
      ...
      <div className="absolute end-1.5 top-1/2 flex -translate-y-1/2 opacity-0 group-focus-within:opacity-100 group-hover:opacity-100">
        ...
        <ThreadListItemPrimitive.Delete asChild>
          <Button
            ...
            size="icon"
            aria-label="删除对话"
            className="text-destructive size-6 p-0"
          >

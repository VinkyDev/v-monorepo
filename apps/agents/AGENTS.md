...
- `pnpm run dev` — Studio + REST at http://localhost:4111. Agent **Editor** and Observability traces are on. AG-UI chat is `POST /agui/:agentId` (Research: `/agui/research-agent`). Memory threads are `/memory/:agentId/threads`.
...
AG-UI runs require `x-mastra-resource-id` (the conversation owner). Working memory is thread-scoped. Thread titles come from the first user message (this custom gateway does not support Mastra `generateTitle`, which needs a non-streaming completion).
